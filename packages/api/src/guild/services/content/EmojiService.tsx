	...
	async createEmoji(
		...
		const fallbackAnimated = hasUnlimitedEmoji
			...
			: useElevatedEmojiLimits
				? MAX_GUILD_EMOJIS_ANIMATED_MORE_EMOJI
				: MAX_GUILD_EMOJIS_ANIMATED;
		const maxStatic = Config.instance.selfHosted
			? MAX_GUILD_EXPRESSION_SLOTS_UNLIMITED
			: this.resolveGuildLimit(staticKey, fallbackStatic, guildFeatures);
		const maxAnimated = Config.instance.selfHosted
			? MAX_GUILD_EXPRESSION_SLOTS_UNLIMITED
			: this.resolveGuildLimit(animatedKey, fallbackAnimated, guildFeatures);
