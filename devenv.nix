{ pkgs, config, lib, ... }:
{
	...
	process.managers.process-compose = {
		...
		settings = {
			...
			processes = {
				...
				livekit = {
					command = lib.mkForce "exec ${config.git.root}/scripts/dev_process_entry.sh livekit ${config.git.root}/scripts/dev_livekit_stack.sh";
	...
	processes = {
		...
		'';
		livekit.exec = ''
			exec ${config.git.root}/scripts/dev_livekit_stack.sh
		'';
		mailpit.exec = ''
