...
generate_livekit_ingress_config() {
    config_path="${FLUXER_CONFIG:-$REPO_ROOT/config/config.json}"
    ingress_config="$REPO_ROOT/dev/livekit_ingress.yaml"
    template="$REPO_ROOT/dev/livekit_ingress.template.yaml"

    info "Generating LiveKit Ingress configuration..."
    ...
    ws_url="ws://${docker_host}:7880"
    redis_address="${docker_host}:6379"

    sed -e "s|{{API_KEY}}|$api_key|g" \
        -e "s|{{API_SECRET}}|$api_secret|g" \
        -e "s|{{WS_URL}}|$ws_url|g" \
        -e "s|{{REDIS_ADDRESS}}|$redis_address|g" \
        "$template" > "$ingress_config"

    info "LiveKit Ingress config generated at: $ingress_config (ws_url: $ws_url)"
}
