...
import {createGuildID, type ChannelID, type GuildID, type UserID} from '../../BrandedTypes';
import type {IGatewayService} from '../../infrastructure/IGatewayService';
import type {ILiveKitService} from '../../infrastructure/ILiveKitService';
import {toHttpUrl} from '../../infrastructure/LiveKitService';
...
function resolveIngressVideoCodec(VideoCodec: Record<string, unknown> | undefined): unknown | null {
	const configured = (process.env.FLUXER_IPTV_INGRESS_VIDEO_CODEC ?? 'vp8').trim().toLowerCase();
	if (!VideoCodec || configured === 'default' || configured === 'default_vc') {
		return null;
	}

	if (configured === 'vp8') return VideoCodec['VP8'];
	if (configured === 'h264' || configured === 'h264_baseline') return VideoCodec['H264_BASELINE'];
	if (configured === 'h264_main') return VideoCodec['H264_MAIN'];
	if (configured === 'h264_high') return VideoCodec['H264_HIGH'];
	return null;
}

function buildIngressVideoOptions(sdk: Record<string, unknown>): unknown | undefined {
	const IngressVideoOptions = sdk['IngressVideoOptions'] as (new (params: Record<string, unknown>) => unknown) | undefined;
	const IngressVideoEncodingOptions = sdk['IngressVideoEncodingOptions'] as
		| (new (params: Record<string, unknown>) => unknown)
		| undefined;
	const TrackSource = sdk['TrackSource'] as Record<string, unknown> | undefined;
	const VideoCodec = sdk['VideoCodec'] as Record<string, unknown> | undefined;
	const videoCodec = resolveIngressVideoCodec(VideoCodec);
	if (!IngressVideoOptions || !IngressVideoEncodingOptions || !TrackSource || videoCodec == null) {
		return undefined;
	}
	...
	return new IngressVideoOptions({
		...
		encodingOptions: {
			...
			value: new IngressVideoEncodingOptions({
				videoCodec,
				frameRate: Number(process.env.FLUXER_IPTV_INGRESS_FRAME_RATE ?? 30),
				layers: [],
			}),
		},
	});
...
async function createIngress(params: {
	...
	const video = buildIngressVideoOptions(sdk);
