...
export const VoiceTab: React.FC<VoiceTabProps> = observer(({voiceSettings, autoRequestPermission = true}) => {
	...
	return (
		<>
			...
			<div className={styles.audioProcessing}>
				<div className={styles.audioProcessingLabel}>
					<Trans>Voice Room Chat</Trans>
				</div>
				<div className={styles.audioProcessingCard}>
					<p className={styles.audioProcessingDescription}>
						<Trans>
							Controls how many chat messages are kept per voice channel and how long they stay in the in-call chat
					...
					<div className={styles.audioProcessingOptions}>
						<div>
							<div className={styles.sliderLabel}>
								<Trans>Message History Limit</Trans>
							</div>
							<Slider
								value={voiceRoomChatMessageLimit}
								defaultValue={voiceRoomChatMessageLimit}
								factoryDefaultValue={120}
								minValue={10}
								maxValue={500}
								step={10}
								markers={[10, 50, 120, 250, 500]}
								stickToMarkers={false}
								onMarkerRender={(value) => `${value}`}
								onValueRender={(value) => `${Math.round(value)}`}
								onValueChange={(value) => VoiceSettingsActionCreators.update({voiceRoomChatMessageLimit: value})}
							/>
						</div>
						<div>
							<div className={styles.sliderLabel}>
							...
							<Slider
								value={voiceRoomChatMessageTtlMinutes}
								defaultValue={voiceRoomChatMessageTtlMinutes}
								factoryDefaultValue={180}
								minValue={5}
								maxValue={1440}
								step={5}
								markers={[5, 30, 60, 180, 720, 1440]}
								stickToMarkers={false}
								onMarkerRender={(value) => `${Math.round(value)}m`}
								onValueRender={(value) =>
									value >= 60 ? t`${Math.round(value / 60)}h` : t`${Math.round(value)}m`
								}
								onValueChange={(value) => VoiceSettingsActionCreators.update({voiceRoomChatMessageTtlMinutes: value})}
							/>
						</div>
					</div>
				</div>
