...
const GuildEmojiTab: React.FC<{guildId: string}> = observer(function GuildEmojiTab({guildId}) {
	...
	const maxStaticEmojis = useMemo(() => {
		if (!guild) return MAX_GUILD_EMOJIS_STATIC;
		if (guild.features.has('MORE_EMOJI')) return MAX_GUILD_EMOJIS_STATIC_MORE_EMOJI;
		return MAX_GUILD_EMOJIS_STATIC;
	}, [guild]);

	const maxAnimatedEmojis = useMemo(() => {
		if (!guild) return MAX_GUILD_EMOJIS_ANIMATED;
		if (guild.features.has('MORE_EMOJI')) return MAX_GUILD_EMOJIS_ANIMATED_MORE_EMOJI;
			...
			{canCreateExpressions && (
				<>
					<UploadSlotInfo
						...
						secondaryUploadButtonText={<Trans>Upload Folder</Trans>}
						onSecondaryUploadClick={() => {
