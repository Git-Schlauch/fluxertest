...
export const VoiceMoreOptionsBottomSheet: React.FC<VoiceMoreOptionsBottomSheetProps> = observer(({isOpen, onClose}) => {
	...
	useEffect(() => {
		if (!isOpen || !isGuildVoiceCall || !connectedChannelId) {
			setIptvStatus(null);
			return;
		}
	...
	}, [connectedChannelId, isGuildVoiceCall, isOpen]);
	...
	if (isGuildVoiceCall && connectedChannelId) {
		menuGroups.push({
			items: [
				{
					icon: <VideoSettingsIcon className={sharedStyles.icon} />,
					label: iptvStatus?.active ? t`Stop IPTV Stream` : t`Start IPTV Stream`,
					onClick: () => {
						if (iptvStatus?.active) {
							void IPTVActionCreators.stopIPTV(connectedChannelId).catch(() => {
						...
						const sourceUrl = window.prompt(t`IPTV source URL (direct stream URL)`);
						...
						void IPTVActionCreators.startIPTVDirect({
							...
							ModalActionCreators.push(
								modal(() => (
									<GenericErrorModal
										title={t`Could not start IPTV stream`}
										message={t`Check the stream URL and make sure you are connected to this voice channel.`}
									/>
								)),
							);
						});
