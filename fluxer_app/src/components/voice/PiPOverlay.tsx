...
export const PiPOverlay = observer(function PiPOverlay() {
	...
	const connecting = MediaEngineStore.connecting;
	const activeChannelId = MediaEngineStore.channelId;
	...
	useEffect(() => {
		if (!room && hasActiveOverlay) {
			PiPStore.close();
			PiPStore.hideFocusedTileMirror();
		}
	}, [hasActiveOverlay, room]);
	...
	useEffect(() => {
		...
		PiPStore.close();
		PiPStore.hideFocusedTileMirror();
	}, [connected, connecting, hasActiveOverlay]);

	useEffect(() => {
		if (!hasActiveOverlay || !content) return;
		if (!activeChannelId) return;
		if (content.channelId !== activeChannelId) {
			PiPStore.close();
			PiPStore.hideFocusedTileMirror();
		}
	}, [activeChannelId, content, hasActiveOverlay]);
