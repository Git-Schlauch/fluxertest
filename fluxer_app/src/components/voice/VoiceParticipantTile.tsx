...
const VoiceParticipantTileInner = observer(function VoiceParticipantTileInner({
	...
	const isIptvParticipant = identity.startsWith('iptv_');
	const isStreamTile = isScreenShare || isIptvParticipant;
	...
	useEffect(() => {
		if (!isStreamTile || isOwnScreenShare || isFocusedPlaceholderTile) return;
		if (!isWatching && !isIptvParticipant) return;
	...
	useEffect(() => {
		if (!isStreamTile || isOwnScreenShare || isFocusedPlaceholderTile) return;
		if (!hasStreamAudioPrefsEntry) return;
	...
	const {
		...
		supportsFullscreen: supportsStreamFullscreen,
		toggleFullscreen: toggleStreamFullscreen,
	} = useMediaFullscreen({
		containerRef: tileRef as React.RefObject<HTMLElement | null>,
		videoRef,
	});

	const showStreamAudioControls = isStreamTile && !isOwnScreenShare && !isCurrentUser;
	const canFullscreenStream = isStreamTile && !isFocusedPlaceholderTile && supportsStreamFullscreen;
	...
	const handleContextMenu = useCallback(
		(event: React.MouseEvent | MouseEvent) => {
			if (!participantUser) {
				if (!showStreamAudioControls && !canFullscreenStream) {
					return;
				}
					...
					<>
						{showStreamAudioControls && (
							<MenuGroup>
								...
								/>
								<CheckboxItem
									icon={<SpeakerSlashIcon weight="fill" className={styles.streamAudioIcon} />}
						...
						{canFullscreenStream && (
							<MenuGroup>
								...
								>
									{isStreamFullscreen ? t`Exit Fullscreen` : t`Enter Fullscreen`}
								</MenuItem>
							</MenuGroup>
						)}
					</>
			...
			<FocusRing offset={-2}>
				<LongPressable
					...
					className={clsx(
						...
						isStreamFullscreen && styles.fullscreenCover,
					)}
					...
					<div className={voiceCallStyles.lkParticipantMetadata}>
						...
						{!isLocalParticipant && (
							...
							>
								...
								{canFullscreenStream && (
									<Tooltip text={isStreamFullscreen ? t`Exit fullscreen` : t`Enter fullscreen`} position="top">
										<FocusRing offset={-2}>
											<div
												role="button"
												tabIndex={0}
											...
											>
												{isStreamFullscreen ? (
													<ArrowsInSimpleIcon weight="bold" className={styles.menuButtonIcon} />
												) : (
													<ArrowsOutSimpleIcon weight="bold" className={styles.menuButtonIcon} />
												)}
											</div>
										</FocusRing>
									</Tooltip>
