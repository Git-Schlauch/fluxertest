...
const IPTV_IDENTITY_PREFIX = 'iptv_';
const IPTV_AUDIO_KEY_PREFIX = 'iptv:';

export function getIptvParticipantAudioKey(channelId: string): string {
	return `${IPTV_AUDIO_KEY_PREFIX}${channelId}`;
}

function getIptvConnectionId(channelId: string): string {
	return `${IPTV_IDENTITY_PREFIX}${channelId}`;
}

function getIptvChannelIdFromIdentity(identity: string): string | null {
	if (!identity.startsWith(IPTV_IDENTITY_PREFIX)) {
		return null;
	}
	const channelId = identity.slice(IPTV_IDENTITY_PREFIX.length).trim();
...
const idUser = (identity: string): string | null => {
	const iptvChannelId = getIptvChannelIdFromIdentity(identity);
	if (iptvChannelId) {
		return getIptvParticipantAudioKey(iptvChannelId);
	}
...
const idConnection = (identity: string): string | null => {
	const iptvChannelId = getIptvChannelIdFromIdentity(identity);
	if (iptvChannelId) {
		return getIptvConnectionId(iptvChannelId);
	}
...
class ParticipantVolumeStore {
	...
	applySettingsToParticipant(participant: RemoteParticipant, selfDeaf: boolean): void {
		...
		if (!userId) return;
		const isIptvParticipant = participant.identity.startsWith(IPTV_IDENTITY_PREFIX);
		...
		participant.audioTrackPublications.forEach((pub) => {
			try {
				...
				const usesStreamAudioPrefs = isScreenShareAudio || isIptvParticipant;
				...
				if (isRemoteAudioTrack(track)) {
					const nextVolume = usesStreamAudioPrefs
				...
				const shouldDisable = locallyMuted || selfDeaf || (usesStreamAudioPrefs && streamMuted);
				...
				if (usesStreamAudioPrefs && streamKey && StreamAudioPrefsStore.hasEntry(streamKey)) {
